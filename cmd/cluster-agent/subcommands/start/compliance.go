...
func startCompliance(stopper startstop.Stopper, apiCl *apiserver.APIClient, isLeader func() bool) error {
	...
	kubernetesProvider := func(ctx context.Context) (dynamic.Interface, error) {
		if isLeader() {
	...
	agent := compliance.NewAgent(compliance.AgentOptions{
		...
		BenchmarkVisitor: filterBenchmarksForKubernetes,
	...
	log.Infof("Running compliance checks")
	return nil
}

func filterBenchmarksForKubernetes(b *compliance.Benchmark) *compliance.Benchmark {
	rules := b.FilterRules(func(r *compliance.Rule) bool {
		return r.HasScope(compliance.KubernetesClusterScope)
	})
	if len(rules) > 0 {
		b.Rules = rules
		return b
