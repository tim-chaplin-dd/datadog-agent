...
func (r *regoCheck) buildNormalInput(env env.Env) (eval.RegoInputMap, error) {
	...
	resolveInput := func(regoInput *regoInput) (err error) {
		...
		defer func() {
			if client := env.StatsdClient(); client != nil {
				tags := []string{
					"rule_id:" + r.ruleID,
					"rule_input_type:" + string(regoInput.Kind()),
					"agent_version:" + version.AgentVersion,
				}
				if err := client.Count(metrics.MetricInputsHits, 1, tags, 1.0); err != nil {
					log.Errorf("failed to send input metric: %v", err)
				}
				if err := client.Timing(metrics.MetricInputsDuration, time.Until(start), tags, 1.0); err != nil {
					log.Errorf("failed to send input metric: %v", err)
				}
			}
		}()
