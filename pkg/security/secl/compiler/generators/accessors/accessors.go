...
func handleBasic(module *common.Module, field seclField, name, alias, aliasPrefix, prefix, kind, event, opOverrides, constants, commentText string, iterator *common.StructField, isArray bool) {
	if verbose {
...
func handleNonEmbedded(module *common.Module, field seclField, prefixedFieldName, event, constants, fieldType string, isPointer, isArray bool) {
	module.AllFields[prefixedFieldName] = &common.StructField{
		Name:          prefixedFieldName,
		Event:         event,
		OrigType:      qualifiedType(module, fieldType),
...
func handleIterator(module *common.Module, field seclField, fieldType, iterator, aliasPrefix, prefixedFieldName, event, fieldCommentText, opOverrides, constants string, isPointer, isArray bool) *common.StructField {
	alias := field.name
	if aliasPrefix != "" {
...
func handleFieldWithHandler(module *common.Module, field seclField, aliasPrefix, prefix, prefixedFieldName, fieldType, typeName, event, fieldCommentText, opOverrides, constants, handler string, isPointer, isArray bool, fieldIterator *common.StructField) {
	alias := field.name
	if aliasPrefix != "" {
		alias = aliasPrefix + "." + alias
	}
	...
	if field.lengthField {
		...
		lengthField.CommentText = "Length of '" + alias + "' string"

		module.Fields[alias+".length"] = &lengthField
	}
	...
	for _, field := range structType.Fields.List {
		...
		if isEmbedded := len(field.Names) == 0; isEmbedded {
			...
			if tags, err := structtag.Parse(string(tag)); err == nil && len(tags.Tags()) != 0 {
				opOverrides, constants, fields = parseTags(tags)

				if opOverrides == "" && constants == "" && fields == nil {
					continue
				}
			...
			for _, seclField := range fields {
				...
				handleNonEmbedded(module, seclField, prefixedFieldName, event, constants, fieldType, isPointer, isArray)

				if seclFieldIterator := seclField.iterator; seclFieldIterator != "" {
					fieldIterator = handleIterator(module, seclField, fieldType, seclFieldIterator, aliasPrefix, prefixedFieldName, event, fieldCommentText, opOverrides, constants, isPointer, isArray)
				}

				if handler := seclField.handler; handler != "" {
					typeName := typeSpec.Name.Name
					handleFieldWithHandler(module, seclField, aliasPrefix, prefix, prefixedFieldName, fieldType, typeName, event, fieldCommentText, opOverrides, constants, handler, isPointer, isArray, fieldIterator)
				...
				if isBasicType(fieldType) {
					handleBasic(module, seclField, fieldBasename, alias, aliasPrefix, prefix, fieldType, event, opOverrides, constants, fieldCommentText, fieldIterator, isArray)
				} else {
...
func parseTags(tags *structtag.Tags) (string, string, []seclField) {
	var opOverrides string
	var constants string
	var fields []seclField
	...
	return opOverrides, constants, fields
}
