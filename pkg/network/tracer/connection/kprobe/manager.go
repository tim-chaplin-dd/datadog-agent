...
var altProbes = map[probes.ProbeName]string{
	...
	probes.TCPSendMsgPre3190:                "kprobe__tcp_sendmsg__pre_3_19_0",
...
func newManager(config *config.Config, closedHandler *ebpf.PerfHandler, runtimeTracer bool) *manager.Manager {
	...
	if runtimeTracer {
		...
		mgr.Probes = append(mgr.Probes,
			...
			&manager.Probe{ProbeIdentificationPair: manager.ProbeIdentificationPair{EBPFSection: string(probes.TCPSendMsgPre3190), EBPFFuncName: altProbes[probes.TCPSendMsgPre3190], UID: probeUID}, MatchFuncName: "^tcp_sendmsg$", KprobeAttachMethod: kprobeAttachMethod},
		)
	}
