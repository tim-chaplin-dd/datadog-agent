...
static __always_inline http2_stream_t *http2_fetch_state(http2_stream_t *http2, http2_packet_t packet_type) {
...
static __always_inline bool http2_seen_before(http2_connection_t *http2, skb_info_t *skb_info) {
...
static __always_inline void http2_update_seen_before(http2_connection_t *http2, skb_info_t *skb_info) {
    if (!skb_info || !skb_info->tcp_seq) {
        return;
    }
...
static __always_inline void http2_begin_request(http2_connection_t* http2_conn, http2_stream_t *http2, http2_method_t method, char *buffer) {
...
static __always_inline __u64 read_var_int(http2_connection_t* http2_conn, __u64 factor){
    if (http2_conn->current_offset_in_request_fragment > sizeof(http2_conn->request_fragment)) {
        return false;
    }
...
static __always_inline bool classify_static_value(http2_stream_t* http2_stream, static_table_entry_t* static_value){
...
#pragma unroll
    for (unsigned headers_index = 0; headers_index < HTTP2_MAX_HEADERS_COUNT; headers_index++) {
        remaining_length = (__s64)sizeof(http2_conn->request_fragment) - (__s64)http2_conn->current_offset_in_request_fragment;
...
#pragma unroll
    for (uint32_t frame_index = 0; frame_index < HTTP2_MAX_FRAMES; frame_index++) {
        ...
        http2_stream_t *http2_stream = http2_fetch_by_stream(&http2_conn->tup, current_frame.stream_id);
