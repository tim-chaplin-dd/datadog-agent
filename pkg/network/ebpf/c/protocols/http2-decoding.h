...
static __always_inline __u64 read_var_int(http2_connection_t* http2_conn, heap_buffer_t *heap_buffer, __u64 factor){
    ...
    __u64 bit_factor = (1 << factor) - 1;
    current_char_as_number &= bit_factor;

    if (current_char_as_number < bit_factor) {
...
static __always_inline void classify_static_value(http2_stream_t* http2_stream, static_table_entry_t* static_value){
    static_table_value_t value = static_value->value;
    if ((static_value->key == kMethod) && (value == kPOST)){
        http2_stream->request_method = value;
    } else if ((value <= k500) && (value >= k200)) {
        http2_stream->response_status_code = value;
    }
}
...
static __always_inline void parse_field_literal(http2_connection_t* http2_conn, heap_buffer_t *heap_buffer, http2_stream_t* http2_stream, dynamic_table_index_t *dynamic_index, dynamic_table_entry_t *dynamic_value){
    ...
    const char *beginning = heap_buffer->request_fragment + heap_buffer->current_offset_in_request_fragment;
    bpf_memcpy(http2_stream->path, beginning, HTTP2_MAX_PATH_LEN);
    http2_stream->path_size = str_len;

    // create the new dynamic value which will be added to the internal table.
    bpf_memcpy(dynamic_value->value.buffer, http2_stream->path, HTTP2_MAX_PATH_LEN);
